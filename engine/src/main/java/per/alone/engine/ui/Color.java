...
public class Color {
    ...
    public static Color parseColor(String color) {
        return parseColor(color, 1.0);
    }

    /**
     * 解析字符串表示的 Color，支持多种表示方法：
     * <table>
     *     <tr>
     *         <th>格式字符串</th>
     *         <th>等效的构造函数</th>
     *     </tr>
     *     <tr>
     *         <td>Color.parseColor("0xff66cc33", 0.5);</td>
     *         <td>new Color(1.0, 0.4, 0.8, 0.1)</td>
     *     </tr>
     *     <tr>
     *         <td>Color.web("rgb(255,102,204)", 0.5);</td>
     *         <td>new Color(1.0, 0.4, 0.8, 0.5)</td>
     *     </tr>
     *     <tr>
     *         <td>Color.web("rgba(255,102,204,0.4)", 0.5);</td>
     *         <td>new Color(1.0, 0.4, 0.8, 0.4)</td>
     *     </tr>
     * </table>
     *
     * @param color 格式颜色字符串
     * @return {@link Color}
     */
    public static Color parseColor(String color, double opacity) {
        if (color.startsWith("#")) {
    ...
    private static Color parseRGBColor(String color, int roff, boolean hasAlpha, double opacity) {
        try {
            ...
            if (aend >= 0) {
                int red = Integer.parseInt(color.substring(roff, rend).trim());
                int green = Integer.parseInt(color.substring(rend + 1, gend).trim());
                int blue = Integer.parseInt(color.substring(gend + 1, bend).trim());
                if (hasAlpha) {
                    double alpha = Double.parseDouble(color.substring(bend + 1, aend).trim());
