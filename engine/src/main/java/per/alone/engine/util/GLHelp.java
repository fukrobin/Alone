...
import per.alone.engine.renderer.ShaderProgram;
...
public class GLHelp {
    ...
    private static final HashMap<String, Integer> LOADED_TEXTURE_CACHE = new HashMap<>();
    ...
    public static void createShader(int shaderType, String renderer, String shaderName) throws IOException {
        if (shaderType == VERTEX_SHADER) {
            currentShaderProgram.createVertexShader(
                    Utils.loadResource("/shader/" + renderer + "/vs_" + shaderName + ".glsl"));
        } else if (shaderType == FRAGMENT_SHADER) {
            currentShaderProgram.createFragmentShader(
                    Utils.loadResource("/shader/" + renderer + "/fs_" + shaderName + ".glsl"));
        }
    }

    public static ShaderProgram setupShaderProgram(String dir, String shaderName,
                                                   String... uniforms) throws IOException {
    ...
    public static int loadTexture(String texturePath) throws IOException {
        if (LOADED_TEXTURE_CACHE.containsKey(texturePath)) {
            return LOADED_TEXTURE_CACHE.get(texturePath);
        }
        ...
        LOADED_TEXTURE_CACHE.put(texturePath, textureId);
        return textureId;
    }
    ...
    public void cleanUp() {
        ...
        for (Integer id : LOADED_TEXTURE_CACHE.values()) {
            glDeleteTextures(id);
        }
