...
@SuppressWarnings("AlibabaClassNamingShouldBeCamel") public class GPUTimer extends AbstractTimer {
        ...
        if (this.supported) {
            ...
            try (MemoryStack stack = MemoryStack.stackPush()) {
                ...
                while (available.get(0) != 0 && this.lastQueryId <= this.currentQueryId) {
                    glGetQueryObjectiv(this.queries.get(this.lastQueryId % GPU_QUERY_COUNT), GL_QUERY_RESULT_AVAILABLE, available);
                    if (available.get(0) != 0) {
                        LongBuffer timeElapsed = stack.mallocLong(1);
                        glGetQueryObjectui64v(this.queries.get(this.lastQueryId % GPU_QUERY_COUNT), GL_QUERY_RESULT, timeElapsed);
                        this.lastQueryId++;
                        if (count < 3) {
