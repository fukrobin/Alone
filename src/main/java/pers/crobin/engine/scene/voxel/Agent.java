...
import pers.crobin.engine.scene.Camera;
 ...
 **/
public class Agent {
    private static final int ALL_PLANE = PLANE_MASK_NX | PLANE_MASK_NY | PLANE_MASK_NZ | PLANE_MASK_PX | PLANE_MASK_PY | PLANE_MASK_PZ;
    ...
    private void addInstanceInfo(OcTree tree) {
        ...
        while (!passTreeList.isEmpty()) {
            ...
            if (current.isLeaf()) {
                ...
                FloatBuffer buffer = behaviorMapInstance.computeIfAbsent(current.nodeData & 0xff, integer -> MemoryUtil.memAllocFloat(4096 * 3));
    ...
    public boolean testChunk(Chunk chunk) {
        ...
        if (chunk.lastCulledPlane == 0) {
            ...
            result = (byte)frustumIntersection.intersectAab(x, 0, z, x + 16, Chunk.HEIGHT_LIMIT, z + 16, ALL_PLANE, chunk.lastCulledPlane);
        }
