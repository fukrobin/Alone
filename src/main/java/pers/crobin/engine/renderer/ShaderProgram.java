...
public class ShaderProgram implements IMemoryManager {
    ...
    public ShaderProgram() {
        ...
        if (programId == 0) {
            throw new RuntimeException("致命错误： 无法创建着色器程序.");
        }
    ...
    public void createUniform(String uniformName) {
        ...
        if (uniformLocation < 0) {
            throw new RuntimeException("致命错误：无法找到uniform变量：" + uniformName);
        }
    ...
    public void bindUniformBlock(String blockName, int bindPoint) {
        ...
        if (blockLocation < 0) {
            throw new RuntimeException("致命错误：无法找到uniform block：" + blockName);
        }
