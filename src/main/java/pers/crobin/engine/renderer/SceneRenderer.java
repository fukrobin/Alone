...
public class SceneRenderer extends BaseRenderer<Scene> implements IMemoryManager {
    ...
    public void initialize() {
        ...
        Model model = new Model("models/standardBlock.obj", "models/");
        ...
        if (target != null) {
            ...
            if (window.isDebugMode()) {
                GlobalVariable.ENGINE_DEBUG_INFO.push(
                        String.format("Position: [x: %.3f, y: %.3f, z: %.3f]", camera.getPosition().x, camera.getPosition().y, camera.getPosition().z));
                GlobalVariable.ENGINE_DEBUG_INFO.push(String.format("Rotation: [x: %.3f, y: %.3f]", camera.getRotation().x, camera.getRotation().y));
            }
