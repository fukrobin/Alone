...
import pers.crobin.engine.kernel.EngineManager;
...
public class Alone extends BaseEngine {
    ...
    private void setScene(EngineManager engineManager) {
        ...
        engineManager.getSceneRenderer().setTarget(scene);
    }
    ...
    @Override
    public void start(String[] args, EngineManager engineManager, Window window) {
        window.setTitle("Scene Model Test");
        window.setSize(1000, 700);
        window.setCenter();
//        setScene(linker);

        GuiManager guiManager = engineManager.getGuiManager();
